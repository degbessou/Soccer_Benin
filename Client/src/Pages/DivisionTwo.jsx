...
export default function DivisionTwo() {
    ...
    useEffect(() => {
        const fetchStandings = async () => {
            const [{ data: stdA }, { data: stdB }] = await Promise.all([
                supabase.from("std_v_div_two_wmen").select("*")
    ...
    const fetchMatchesByPoule = (poule) => async () => {
        const { data } = await supabase
            ...
            .eq("ligue", "D2 Féminine").eq("poule", poule)
    ...
    const fetchStandingByPoule = (poule) => async () => {
        const { data } = await supabase
            .from("std_v_div_two_wmen").select("*")
    ...
    return (
        <>
            ...
            <TitleBis title="Tout savoir sur la D2 Féminine, 2025-2026" />
                        ...
                        {poules.map(poule => (
                            <PouleContent key={poule.value} value={poule.value}>
                                ...
                                <Schedule
                                    ...
                                    externalDownloadFilename={`calendrier-D2Féminine-poule${poule.value}.png`}
                                />
                            </PouleContent>
                        ))}
                    </PouleTabs>
                        ...
                        {poules.map(poule => (
                            <PouleContent key={poule.value} value={poule.value}>
                                <StandingPool
                                    ...
                                    externalDownloadFilename={poule.value === 'A' ? `classement-d2f-j${journeesJoueesA}.png` : undefined}
            ...
            <StandingPoolCapture
                ...
                logoUrl={getSupabaseImageUrl('medias/icons/logo_no.png')}
                title="Classement D2 Féminine / 2025-2026"
                subtitle="www.bencofoot.com"
