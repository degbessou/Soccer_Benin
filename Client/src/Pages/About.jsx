        ...
        <div className="max-w-screen-lg mx-auto px-4 md:px-8">
          ...
          <div className="mt-12 divide-y sm:mt-20">
            {faqsList.map((list, idx) => (
              <div
                key={idx}
                className="py-5 first:pt-0 sm:grid sm:grid-cols-4 sm:gap-8"
              >
                <div className="max-w-sm pt-0 pb-6 sm:pt-0 lg:flex-grow">
                  <h4 className="text-green-700 font-semibold">{list.label}</h4>
                </div>
                <ul className="flex-1 space-y-6 sm:col-span-3 sm:last:pb-6 sm:space-y-8">
                  {list.qas.map((item, idx) => (
                    <li key={idx}>
                      <summary className="flex items-center justify-between font-semibold text-yellow-700">
                        {item.q}
                      </summary>
                      <p
